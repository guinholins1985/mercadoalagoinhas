...
const StatCard: React.FC<{title: string; value: string | number; icon: React.ReactNode, themeClasses: ReturnType<typeof getThemeClasses>}> = ({ title, value, icon, themeClasses }) => (
    <div className="bg-white p-6 rounded-lg shadow-md flex items-center gap-4">
        <div className={`${themeClasses.bg100} p-3 rounded-full ${themeClasses.text600}`}>
            {icon}
        </div>
        <div>
    ...
    return (
        <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                <StatCard title="Total de Produtos" value={totalProducts} icon={<PackageIcon />} themeClasses={theme} />
                <StatCard title="Total de Vendedores" value={totalSellers} icon={<UsersIcon />} themeClasses={theme} />
                <StatCard title="Vendedores Pendentes" value={pendingSellers} icon={<ClockIcon />} themeClasses={theme} />
                <StatCard title="Assinaturas Ativas" value={activeSubscriptions} icon={<CreditCardIcon />} themeClasses={theme} />
            </div>
                    ...
                    <ul className="space-y-4">
                        {topProducts.map(p => (
                             <li key={p.id} className="flex items-center gap-4">
                                ...
                                <p className={`font-bold ${theme.text600}`}>R$ {p.price.toFixed(2)}</p>
                            </li>
                        ))}
                    </ul>
                </div>
                        ...
                        {sellerPerformance.map(s => (
                             <li key={s.id}>
                                ...
                                <div className="w-full bg-slate-200 rounded-full h-2.5">
                                    <div 
                                        className={`${theme.bg500} h-2.5 rounded-full`} 
